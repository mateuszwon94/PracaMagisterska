...
Po lewej stronie znajdują się poszczególne lekcje. Możesz wybrać dowolną, ale zdecydowanie zalecam wybieranie ich i rozwiązywanie po kolei. Zapewne dość często będzie sie zdarzać, że jedna z lekcji będzie wymagała wiedzy z poprzednich. Tak to już jest z programowaniem, że bez podstaw nie da się zrobić rzeczy bardziej zaawansowanych.
Jeżeli dla lekcji istnieją jakieś testy, możesz je uruchomić odpowiednim przyciskiem. Jeśli wszystkie testy zakończą się pozytywnie, Twoje rozwiązanie zostanie ocenione zarówno pod kątem liczby linii kodu, jak i czasu wykonania.
Rodzaje testów, z jakimi się spotkasz:
- Test statyczny - sprawdzają, czy użyłeś właściwych konstrukcji (np. czy w lekcji z warunkami użyłeś konstrukcji z if-em).
- Proste testy - kilka prostych testów pokazujących, jak Twoje rozwiązanie ma się zachowywać w konkretnych sytuacjach.
- Testy rzeczywiste - sporo testów, które zawierają w sobie np. testowanie wartości granicznych. Mają one dać pełen obraz tego, czy Twoje rozwiązanie jest poprawne.
- Testy losowe - mają po prostu za każdym razem losowo wygenerowane parametry (oczywiście takie, które nie znajdują się w testach prostych czy rzeczywistych). Mają na celu głównie zabezpieczenie się przed programami napisanymi tak, by przechodzić testy, a nie tak, żeby być kompleksowymi i poprawnymi rozwiązaniami.
